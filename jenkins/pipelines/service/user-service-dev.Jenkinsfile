pipeline {
  ...
  stages {
    stage('Clean workspace') {
      steps { script { deleteDir(); echo 'Workspace cleaned.' } }
    }
    stage('Checkout') { steps { checkout scm } }

    stage('Build') {
      steps { dir("${SERVICE_NAME}") { sh "mvn -B -e -DskipTests clean package ${MAVEN_OPTS}" } }
    }

    stage('Unit Tests') {
      steps { dir("${SERVICE_NAME}") { sh "mvn -B -e -DfailIfNoTests=false test ${MAVEN_OPTS}" } }
    }
    ...
    stage('Deploy to Dev (optional)') {
      steps { echo "Deploy steps depend on your environment; configure job to perform deployment if desired." }
    }
  }
