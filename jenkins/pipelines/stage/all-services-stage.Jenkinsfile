            ...
            steps {
                script {
                    ...
                    if (params.FORCE_ALL.toString() == 'true') {
                        ...
                        def remoteBase = sh(returnStdout: true, script: "bash -lc 'for b in dev master main; do if git ls-remote --heads origin $b | grep $b >/dev/null 2>&1; then echo $b; break; fi; done' ").trim()
                        if (remoteBase) {
